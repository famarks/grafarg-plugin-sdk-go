...
func ReloadPlugin() error {
	...
	// Wait for grafarg to start plugin
	for i := 0; i < 20; i++ {
		...
		log.Printf("waiting for grafarg to start: %s...", exeName)
	}
	return nil
}
...
//
// The plugin process is killed after re-building, in order to make Grafarg launch the new version. Once the new
...
func Debugger() error {
	...
	for i := 0; i < 20; i++ {
		...
		log.Printf("Waiting for Grafarg to start plugin: %q...", exeName)
		time.Sleep(250 * time.Millisecond)
	}
	if pid == -1 {
		return fmt.Errorf("could not find plugin process %q, perhaps Grafarg is not running?", exeName)
	}
