...
import (
	...
	"github.com/famarks/grafarg-plugin-sdk-go/backend/grpcplugin"
	"github.com/famarks/grafarg-plugin-sdk-go/backend/log"
	"github.com/famarks/grafarg-plugin-sdk-go/genproto/pluginv2"
	"github.com/famarks/grafarg-plugin-sdk-go/internal/standalone"
	"github.com/famarks/grafarg-plugin-sdk-go/internal/tracerprovider"
)
...
func GracefulStandaloneServe(dsopts ServeOpts, info standalone.ServerSettings) error {
	...
	defer func() {
		...
		// Kill the dummy locator so Grafarg reloads the plugin
...
func Manage(pluginID string, serveOpts ServeOpts) error {
	...
	if s, enabled := standalone.ClientModeEnabled(pluginID); enabled {
		// Grafarg is trying to run the dummy plugin locator to connect to the standalone GRPC server (separate process)
