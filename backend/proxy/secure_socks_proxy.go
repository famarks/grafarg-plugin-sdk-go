...
// SecureSocksProxyEnabled checks if the Grafarg instance allows the secure socks proxy to be used
// and the datasource options specify to use the proxy
func (p *cfgProxyWrapper) SecureSocksProxyEnabled() bool {
	// it cannot be enabled if it's not enabled on Grafarg
	if p.opts == nil {
		return false
	}

	// if it's enabled on Grafarg, check if the datasource is using it
...
// if it is enabled on the datasource and the grafarg instance
...
// getConfigFromEnv gets the needed proxy information from the env variables that Grafarg set with the values from the config ini
