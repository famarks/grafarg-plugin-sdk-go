...
1. The user enables the	HTTP logger for the GitHub data source plugin in their `grafarg.ini` configuration file.
...
To enable the HTTP logger for a plugin, add the following to your `grafarg.ini` configuration file:

```
[plugin.grafarg-github-datasource]
har_log_enabled = true
har_log_path = /home/example/github.har
```

In the example above, `grafarg-github-datasource` is the plugin ID, which can be found in the plugin's `src/plugin.json` file.
...
func getClient() (*http.Client, error) {
	...
	h := http_logger.NewHTTPLogger("grafarg-example-datasource", client.Transport)
	client.Transport = h
	return client, nil
}
```

In the example above, `grafarg-example-datasource` is the plugin ID, which should match the `id` property in the `src/plugin.json` file.
