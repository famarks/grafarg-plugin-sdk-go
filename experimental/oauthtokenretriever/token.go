...
func New() (TokenRetriever, error) {
	// The Grafarg URL is required to obtain tokens later on
	grafargAppURL := strings.TrimRight(os.Getenv(AppURL), "/")
	if grafargAppURL == "" {
		// For debugging purposes only
		grafargAppURL = "http://localhost:3000"
	}
	...
	return &tokenRetriever{
		...
		conf: &clientcredentials.Config{
			...
			TokenURL:     grafargAppURL + "/oauth2/token",
