...
import (
	...
	"github.com/famarks/grafarg-plugin-sdk-go/experimental/mock"
...
func TestRoundTripper_RoundTrip(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			req:     exampleRequest(http.MethodGet, "https://grafarg.com/api/plugins/two"),
		...
		},
		{
			...
			rt:      &mock.RoundTripper{HARFileName: "testdata/example.har"},
			req:     exampleRequest(http.MethodGet, "https://grafarg.com/api/plugins"),
			wantErr: errors.New("no matched request found in HAR file"),
		},
		{
		...
		},
		{
			...
			rt:   &mock.RoundTripper{BasicAuthEnabled: true, BasicAuthUser: "foo", BasicAuthPassword: "bar"},
			req:  exampleRequest(http.MethodGet, "https://foo:bar@grafarg.com/api/plugins"),
			test: func(t *testing.T, res *http.Response) {
				t.Helper()
